...
const AdminLibrary = () => {
  ...
  const { tracks, folders, removeTrack, updateTrack, addTrack, assignToFolder, finalTracks, addToFinal, removeFromFinal } = useStudio();
  ...
  const handleEdit = (track: Track) => {
    ...
    if (newTitle) {
      updateTrack(track.id, { title: newTitle });
  ...
  const filterStyles = ['All', 'Samba', 'Cha-cha-cha', 'Rumba', 'Jive', 'Slow Waltz'];
    ...
    <div className="admin-library animate-in">
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredTracks.map((track) => {
                ...
                return (
                  <tr key={track.id} className={`track-row ${isThisPlaying ? 'is-playing-row' : ''}`}>
                    ...
                    <td className="col-style">
                      <div className="style-actions">
                        ...
                        <button 
                          className={`row-flag-btn ${finalTracks.some(t => t.id === track.id) ? 'active' : ''}`}
                          onClick={() => finalTracks.some(t => t.id === track.id) ? removeFromFinal(track.id) : addToFinal(track)}
                          title="Final Mode Playlist"
                        >
      ...
      <PlaylistModal 
        ...
        onConfirm={(folderId, folderName) => {
          if (selectedTrack) {
            assignToFolder(selectedTrack.id, folderId);
      ...
      <style jsx>{`
        ...
        .action-btn { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; color: #71717a; transition: all 0.2s; }
